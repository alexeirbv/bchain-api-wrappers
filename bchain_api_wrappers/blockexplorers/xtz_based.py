...
class XtzBased(abstract.AbstractAdapter):
    ...
    async def __get_height(self):
        resp = await self._send_http_request(
            url=f"{self.url}/explorer/status"
        )
        if not self.service_status.error_code:
            try:
                height = int(resp["response"]["blocks"]) if self.is_provider else int(resp["response"]["indexed"])
