...
class OneBased(abstract.AbstractAdapter):
    ...
    async def __get_height(self):
        resp = await self._send_http_request(
            url=self.url,
            method="POST",
            body=json.dumps({"jsonrpc": "2.0", "id": "chainvisor", "method": "hmy_latestHeader", "params": []}),
        )

        if not self.service_status.error_code:
            try:
                height = int(resp["response"]["result"]["blockNumber"])
